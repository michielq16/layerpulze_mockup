// H2 — Cost Attribution + Workload Mix
...
// ─── Capacity selector ────────────────────────────
function CapacitySelector({ caps, value, onChange }) {
...
// ─── Capacity (refreshed with selector + per-capacity correctness) ─
function Capacity() {
...
// ─── Cost Attribution tab (Story 1) ──────────────
function CostAttribution({ capId }) {
  const cap = DATA.capacities.find(c => c.id === capId);
...
function Treemap({ groups, totalCU, cap }) {
  const W = 900, H = 360;
...
// ─── Workload Mix tab (Story 2) ──────────────────
function WorkloadMix({ capId }) {
  const cap = DATA.capacities.find(c => c.id === capId);
...
Object.assign(window, { Capacity, Costs, CapacitySelector, CostAttribution, WorkloadMix, Observations, Treemap });
