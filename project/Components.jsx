...
function StatCard({ label, value, unit, sub, delta, icon, tone = 'sky', spark }) {
  return (
...
function Sparkline({ data, tone = 'sky', w = 80, h = 28 }) {
  const color = {
...
function Provenance({ level = 'silver' }) {
  const cfg = {
...
function IssueCard({ id, severity, category, title, evidence, impact, impactTone, onClick }) {
  return (
...
// Novel visual: proportional contribution ribbon (stacked weighted blocks)
function HealthRibbon({ composition, score }) {
  const tones = {
  ...
  // contribution = (value/100 * weight) — share of the 100 score each dim contributes
...
// Topbar + breadcrumb
function Topbar({ crumbs, tenant, onTheme, theme, onTweaks }) {
  return (
...
Object.assign(window, { StatCard, Sparkline, Badge, EnvBadge, Provenance, IssueCard, HealthRibbon, Sidebar, Topbar });
